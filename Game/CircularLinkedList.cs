...
namespace Game
{
	...
	public sealed class CircularLinkedList<T> : ICollection<T>, IEnumerable<T>
	{
		...
		/// <param name="item">Item to search</param>
		/// <returns><see cref="Node&lt;T&gt;"/> instance or NULL</returns>
		public Node<T> FindSkipped(int skipped)
		{
			Node<T> node = FindNodeSkipped(head, skipped);
			return node;
		}
		...
		Node<T> FindNodeSkipped(Node<T> node, int skipped)
		{
			Node<T> result = null;
